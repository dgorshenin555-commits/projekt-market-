/* test_landing.jsx — alternative SaaS-style homepage (TEST variant, isolated from the app) */
(function () {
  const Icon = window.Icon, FuncMark = window.FuncMark, Mascots = window.Mascots;
  ...
  /* ---- live product-interface preview (auto-cycling sections) ---- */
  function Pill({ color, children }) {
    return <span className="tl-prev__pill" style={{ color }}>{children}</span>;
  }

  /* avatar: image with graceful initials fallback (à la shadcn/Radix Avatar) */
  ...
  function Preview() {
    ...
    // park the cursor on the active item on mount
    React.useEffect(() => { moveCursorTo(active); /* eslint-disable-next-line */ }, []);

    // sequence: wait → glide cursor to next item → click → switch section
    React.useEffect(() => {
      if (paused) return;
      let t2, t3;
      const t1 = setTimeout(() => {
  ...
  /* scroll-linked 3D tilt wrapper (Aceternity-style: card lies back, flattens on scroll) */
  ...
  /* hero prompt / search box (adapted from the ChatGPT prompt-input component) */
  ...
  /* benchmark-style metric card (adapted from the performance-benchmark prompt) */
  function PerfCard({ go, icon, title, tint, cap, value, change, bench, benchLabel, compTitle, comp, action }) {
    ...
    React.useEffect(() => {
      ...
      const run = () => {
        ...
        fb = setTimeout(() => setN(value), dur + 400); // guarantee final value even if rAF is throttled
      };
      let io;
      ...
      late = setTimeout(() => { if (!done) run(); }, 2600); // run anyway if IO never fires
    ...
    const cards = [
      {
        ...
        levelsTitle: "Уровни рейтинга", levels: [["0", 800, "#f4717f"], ["4.0", 1100, "#f5b13d"], ["4.5", 1400, "#facc15"], ["4.8+", 2500, "#34d399"]],
      ...
      },
      {
        ...
        levelsTitle: "Сроки прохождения, дн.", levels: [["0", 15, "#34d399"], ["15", 30, "#facc15"], ["30", 45, "#f5b13d"], ["45+", 60, "#f4717f"]],
      ...
      },
      {
        ...
        levelsTitle: "Покрытие BIM, %", levels: [["0", 40, "#f4717f"], ["40", 70, "#f5b13d"], ["70", 90, "#facc15"], ["90+", 100, "#34d399"]],
  ...
  /* --- step visuals (mini mockups of the platform) for the carousel --- */
  function VPublish() {
  ...
  function Steps() {
    ...
    // move the fake cursor to the marked target on the current slide, then "click" it (highlight stays)
    React.useEffect(() => {
      let t1, t2, t3, el;
      t1 = setTimeout(() => {
  ...
  function TestLanding({ go }) {
    ...
    return <main className="tl" ref={rootRef}><Nav go={go} /><Hero go={go} /><Features go={go} /><ObjectTypes go={go} /><Steps /><window.RegionMap go={go} /><Pricing go={go} /><FAQ /><Band go={go} /><Footer /></main>;
  }

  window.TestLanding = TestLanding;
})();
